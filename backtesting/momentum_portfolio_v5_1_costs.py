...
def close_position(
    ...
    """
    Ferme une position et calcule exactement
    les coûts, le produit net et le PnL.
    """
...
def run_backtest(full_df, hold_days):
    ...
    for current_date in dates:
        ...
        for (
            ...
            trade, net_proceeds = close_position(
                position,
                raw_exit_price,
                current_date,
                reason
    ...
    if positions:
        ...
        # ----------------------------------------------------
        # AJOUT DE LA VRAIE EQUITY FINALE
        # APRES LIQUIDATION
        # ----------------------------------------------------
    ...
    # Plusieurs lignes peuvent avoir la même
    # date finale après liquidation.
    # On garde la dernière, qui correspond
    # à l'equity réellement liquidée.
...
print("=" * 90)
print("MOMENTUM V1 - PORTFOLIO V5.1")
print("FRAIS + SLIPPAGE - COMPTABILITE CORRIGEE")
print("=" * 90)
...
for hold_days in HOLD_DAYS:
    ...
    trades.to_csv(
        f"results/momentum/"
        f"momentum_portfolio_v5_1_costs_trades_"
        f"{hold_days}d.csv",
        index=False
    )
    ...
    equity.to_csv(
        f"results/momentum/"
        f"momentum_portfolio_v5_1_costs_equity_"
        f"{hold_days}d.csv",
        index=False
    )
...
summary.to_csv(
    "results/momentum/"
    "momentum_portfolio_v5_1_costs_summary.csv",
    index=False
)

print()
print("=" * 90)
print("COMPARAISON FINALE V5.1")
print("=" * 90)
