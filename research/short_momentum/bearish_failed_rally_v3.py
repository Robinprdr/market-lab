#!/usr/bin/env python3
"""Bearish Failed Rally V3: final fixed intraday-rejection hypothesis.
Run from repo root: python -B research/short_momentum/bearish_failed_rally_v3.py --cache-dir /tmp/market-lab-bfr-cache --output-dir /tmp/market-lab-bfr-v3
Prior context: Close[t-1]<SMA50[t-1], Return_10D[t-1]<0, Close[t-1]>Close[t-2].
Rejection: High[t]>High[t-1], Close[t]<Open[t], Close[t]<Close[t-1],
positive daily range and (Close[t]-Low[t])/(High[t]-Low[t])<=0.50.
Enter Open[t+1]; action return Close[t+h]/Open[t+1]-1, h=1,3,5 sessions.
No optimization, no additional filter, no costs. SMA20 is calculated only
to reconcile existing core data, never used by the signal.
Dependencies: numpy, pandas. Reuses cached OHLC and committed core labels.
"""
...
def signal_features(frame):
    ...
    x["Close_Location"] = (x.Close - x.Low) / x.Daily_Range.where(x.Daily_Range.gt(0))
    x["Signal"] = (
        x.Prior_Distance_SMA50.lt(0)
        & x.Prior_Return_10D.lt(0)
        & x.Prior_Close.gt(x.Prior_Prior_Close)
        & x.High.gt(x.Prior_High)
        & x.Close.lt(x.Open)
        & x.Daily_Range.gt(0)
        & x.Close_Location.le(0.50)
...
def self_test():
    close = list(np.linspace(120, 90, 65)) + [91.0, 90.0]
    ...
    i = len(x)-1
    x.loc[i, ["Open", "High", "Low"]] = [91.0, 92.0, 88.0]
    assert signal_features(x).Signal.iloc[-1]  # exact 0.50 accepted
    cases = [
        (i, "Low", 87.9),  # close above midpoint: rejected
        (i, "High", x.High.iloc[-2]),  # equal high: rejected
        (i, "Open", 90.0),  # doji: rejected
        (i, "Close", 91.0),  # no decline vs prior close
        (i-1, "Close", 90.0),  # no prior rebound
    ]
    ...
    y = x.copy()
    y.loc[i, ["Open", "High", "Low", "Close"]] = 90.0
    z = signal_features(y)
    assert not z.Signal.iloc[-1] and pd.isna(z.Close_Location.iloc[-1])
    y = x.copy()
    y.loc[:54, "Close"] = 50.0
    assert not signal_features(y).Signal.iloc[-1]
...
def independent_signal_check(x):
    c, high, low, op = [x[k].to_numpy() for k in ("Close", "High", "Low", "Open")]
    ...
    for i in range(50, len(x)):
        ...
        expected[i] = (
            ...
            and c[i-1] > c[i-2]
            and high[i] > high[i-1] and c[i] < op[i] and c[i] < c[i-1]
            and daily_range > 0 and (c[i]-low[i])/daily_range <= 0.50
        )
...
def main():
    ...
    assert (signals.High > signals.Prior_High).all()
    assert (signals.Close < signals.Open).all()
    assert (signals.Daily_Range > 0).all()
    assert (signals.Close_Location <= 0.50).all()
    ...
    stats = metrics(joined, "BASELINE_GLOBAL") + metrics(signals, "BFR_V3")
    for year in sorted(joined.Year.unique()):
        stats += metrics(joined[joined.Year.eq(year)], "BASELINE_GLOBAL", int(year))
        stats += metrics(signals[signals.Year.eq(year)], "BFR_V3", int(year))
    ...
    keep = ["Date", "Ticker", "Episode_ID", "Rebound_Date", "Prior_Close", "Prior_Prior_Close", "Prior_High", "Daily_Range", "Close_Location", "Open", "High", "Low", "Close", "SMA50", "Prior_Distance_SMA50", "Prior_Return_10D", "Session", "Entry_Date", "Entry_Open"]
    ...
    summary = {"commit": COMMIT, "core_git_blob": CORE_BLOB, "ohlc_sha256": hashlib.sha256(raw_path.read_bytes()).hexdigest(),
               ...
               "zero_range_rows": int(joined.Daily_Range.eq(0).sum()),
    ...
    print(stats[stats.Group.eq("BFR_V3") & ~stats.Year.eq("ALL")].to_string(index=False))
